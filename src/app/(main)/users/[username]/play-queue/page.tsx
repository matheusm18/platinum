...
export default async function PlayQueuePage({ params }: Props) {
  ...
  const [playQueueItems, followRow] = await Promise.all([
    db
      .select({ queueItem: playQueue, game: games })
      .from(playQueue)
          ...
          <div className="flex-w-full flex-col items-center justify-between gap-4 md:flex-row md:items-start">
            ...
            <div className="mt-3 flex flex-wrap justify-center gap-x-6 gap-y-3 md:justify-start">
              <div className="flex items-baseline gap-1.5">
                <span className="font-semibold text-white">{playQueueItems.length}</span>
                <span className="text-silver-dim ml-1.5 text-sm">jogos na fila</span>
              </div>
            </div>
          </div>
        </div>
